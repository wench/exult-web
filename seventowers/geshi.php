 ...
 * directory.
 *
 *	THIS FILE HAS BEEN MODIFIED! In order to improve the highlighting for
 *	Usecode C files, a minor alteration was done to this file.
 *
...
class GeSHi
{
	...
	function parse_non_string_part (&$stuff_to_parse)
	{
		...
		//
		if ($this->lexic_permissions['NUMBERS'] && preg_match('#0[xX][A-Fa-f0-9]+#', $stuff_to_parse )) {
			//$stuff_to_parse = preg_replace('#([^a-zA-Z0-9_\#])([0-9]+)([^a-zA-Z0-9])#', "\\1<|/NUM!/>\\2|>\\3", $stuff_to_parse);
			//$stuff_to_parse = preg_replace('#([^a-zA-Z0-9_\#>])([0-9]+)([^a-zA-Z0-9])#', "\\1<|/NUM!/>\\2|>\\3", $stuff_to_parse);
			$stuff_to_parse = preg_replace('/(\\b0[xX][A-Fa-f0-9]+\\b)/', '<|/NUM!/>\\1|>', $stuff_to_parse);
		}
